...
pub fn init(bind: &str) -> Result<MetricsHandle> {
    ...
    metrics::gauge!(metric_names::PROCESS_START_TIME).set(
...
fn prime_event_counters() {
    ...
    for outcome in ["applied", "duplicate", "stale", "unmatched", "foreign"] {
        metrics::counter!(metric_names::BILLING_WEBHOOKS, "outcome" => outcome).increment(0);
    }
    for reason in ["signature", "malformed", "failed", "stalled"] {
        metrics::counter!(metric_names::BILLING_WEBHOOK_REJECTED, "reason" => reason).increment(0);
    }
