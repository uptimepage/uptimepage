...
pub struct IncidentDetail {
    ...
    /// Pages an incident with no monitor is posted to. Publish replaces the
    /// whole list, so send this back with your change. Empty for a monitor's
    /// incident, which shows on the pages carrying that monitor.
...
pub struct PublishBody {
    ...
    /// Replaces the pages an incident with no monitor appears on; omitted
    /// keeps them. Refused for an incident with a monitor.
    #[serde(default)]
    pub status_page_ids: Option<Vec<Uuid>>,
}
...
pub async fn get_incident(
    ...
    let status_page_ids = state.incident_ops_store.status_pages(org, id).await?;
...
pub async fn declare_incident(
    ...
    if publish && new.target_id.is_none() && new.status_page_ids.is_empty() {
        return Err(AppError::bad_request_field(
            codes::INCIDENT_STATUS_PAGE_REQUIRED,
...
#[utoipa::path(
    ...
    summary = "Publish an incident to its status pages",
    description = "Flips visibility to public so the incident shows on any status page \
                   whose components include its monitor. An incident with no monitor shows \
                   on the pages named in `status_page_ids` instead, and needs at least one. \
                   Optionally seeds the public title/description; omitted fields keep their \
                   stored value.",
