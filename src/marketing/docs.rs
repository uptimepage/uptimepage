...
pub struct DocPage {
    ...
    pub section: Section,
    pub scope: Scope,
    /// `datePublished`; never later than `lastmod`.
...
mod tests {
    ...
    #[test]
    fn every_doc_is_published_no_later_than_its_lastmod() {
        for doc in DOCS {
            for date in [doc.created, doc.lastmod] {
                assert!(
            ...
            assert!(
                doc.created <= doc.lastmod,
                "{}: created {} is after lastmod {}",
                doc.slug,
                doc.created,
                doc.lastmod
            );
