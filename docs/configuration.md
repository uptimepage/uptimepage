...
The notification destination points at `{public_base_url}/hooks/billing/paddle` and needs these events: `subscription.created`, `subscription.updated`, `subscription.canceled`, `subscription.paused`, `subscription.resumed`, `subscription.past_due`, `subscription.activated`, `transaction.completed`, `transaction.payment_failed`, `adjustment.created` and `adjustment.updated`. Every completed payment, and every status change of a refund, chargeback or chargeback reversal, is written to the account's billing ledger (`payment_received`, `refund_recorded`) with Paddle's transaction and adjustment ids and the amount. A refund is tied to its account through the subscription it names or the payment it reverses; one that matches neither is acknowledged with a warning naming both Paddle ids. A refund never changes the plan by itself: ending the subscription stays a support decision, taken in Paddle. An approved refund of a whole charge on a live subscription logs a warning so it is not missed.
