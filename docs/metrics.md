...
| `uptimepage_host_throttle_waits_total{kind}` | counter | per-(org,host,port) (`kind=host`) or per-TLD registry (`kind=rdap`) throttle acquire attempts. `host` fails fast over cap; `rdap` queues for the slot. The `rdap` label covers WHOIS lookups too, since both share the per-TLD bulkhead |
| `uptimepage_host_throttle_drops_total` | counter | host-bulkhead rejections — an over-cap `(org, host, port)` tick is dropped with no result written and no alert. Registry lookups never drop: over cap they queue for the per-TLD slot, and one that runs out of time surfaces as `rdap timeout` (see `domain_expiry_stale_served_total{kind="timeout"}`) |
...
| `uptimepage_domain_expiry_stale_served_total{kind}` | counter | times the domain-expiry executor served a cached last-good answer instead of a fresh probe. `kind` distinguishes the cause: `timeout`, `lookup_error`, `unsupported_tld`, or `fresh_error` (no usable last-good — emitted as a real `Error` instead) |
...
| `uptimepage_billing_webhooks_total{outcome}` | counter | payment-provider webhooks accepted, by `outcome`. `applied` is normal; `duplicate` and `stale` are the provider's own redelivery and reordering and are expected. `unmatched` (a payment or subscription event naming an account we do not know, or naming one under a checkout tag that is not ours; events that carry no account by nature, such as a customer being created, count as `applied`) and `foreign` (a subscription that is not the account's live one) should not be routine |
| `uptimepage_billing_webhook_rejected_total{reason}` | counter | payment-provider webhooks not acted on, by `reason`. `signature` at a steady trickle means the endpoint secret in config disagrees with the provider's; `malformed` did not parse, so the payload shape changed and a replay repeats the same bytes; `failed` answered 5xx so the provider retries, and a sustained rate means every retry hits the same wall, usually a price the `plan_prices` table does not carry; `stalled` answered 503 because the delivery could not be committed inside the provider's five-second patience: the provider's own API or the database answering slowly, or, the usual case, the delivery queued behind a plan change the same account had in flight, since one account's changes and webhooks take turns; the redelivery heals it on its own |
...
| `uptimepage_billing_provider_date_failed_total` | counter | price changes booked for the next period that may have left the provider's billing date moved: the provider restarted the billing period (a price on the other cadence does) and the call putting the date back to the paid period end failed twice or was refused, or the change itself went unanswered and could not be confirmed. The subscription may then bill a whole cadence early or late until the date is checked and set by hand in the provider dashboard; the app log line `billing date left moved by a price change` names the subscription and the date it should carry. Should sit at zero |
