...
| `uptimepage_billing_webhooks_total{outcome}` | counter | payment-provider webhooks accepted, by `outcome`. `applied` is normal; `duplicate` and `stale` are the provider's own redelivery and reordering and are expected. `unmatched` (an account we do not know) and `foreign` (a subscription that is not the account's live one) should not be routine |
