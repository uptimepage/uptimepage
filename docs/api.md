...
| `GET` | `/api/v1/account/billing` | subscription status, plan, period end, the cadence it bills on (`interval`), and the plans on sale with their price (`amount_minor`, `currency`) per cadence (session only) |
